...
for i, user_data in enumerate(users_data, start=1):
    for film, rating in zip(user_data['film_adı'], user_data['puan']):
        user_movie_matrix.loc[f'Kullanıcı{i}', film] = rating
...
for i, user_data in enumerate(users_data, start=1):
    ...
    user_data_copy.pop('tercihler')  # Remove the 'tercihler' key
    print(pd.DataFrame(user_data_copy))
